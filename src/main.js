...
ipcMain.handle('admin:getAllUsers', async (event) => {
    try {
        const users = await db.players.getAllPlayers({ limit: 100 });
        return users;
    } catch (err) {
        console.error("Admin get all users failed:", err);
        return { error: err.message };
    }
});

// Admin: Update User Role
ipcMain.handle('admin:updateUserRole', async (event, { userId, roleId }) => {
    try {
        const result = await db.players.updatePlayerRole({ profileId: userId, roleId });
...
ipcMain.handle('admin:updateUserBalance', async (event, { userId, balance }) => {
    try {
        const result = await db.players.updateBalance({ profileId: userId, newBalance: balance });
        return { success: true, changes: result.changes };
    } catch (err) {
        console.error("Admin update user balance failed:", err);
        return { success: false, error: err.message };
    }
});

// Admin: Reset User History
ipcMain.handle('admin:resetUserHistory', async (event, { userId }) => {
    try {
        const result = await db.players.resetPlayerHistory(userId);
        return { success: true, deletedRounds: result.deletedRounds };
    } catch (err) {
        console.error("Admin reset user history failed:", err);
        return { success: false, error: err.message };
    }
});
...
ipcMain.handle('admin:getUserStatistics', async (event, { profileId, startDate, endDate }) => {
    try {
        ...
        console.error("Admin get user statistics failed:", err);
        return { success: false, error: err.message };
    }
});

// Admin: Get All Users Statistics
ipcMain.handle('admin:getAllUsersStatistics', async (event, { startDate, endDate }) => {
    try {
        const allStats = await db.statistics.getAllUsersStatistics({ startDate, endDate });
        return allStats;
    } catch (err) {
        console.error("Admin get all users statistics failed:", err);
        return { error: err.message };
    }
});
